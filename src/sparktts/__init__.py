...
class SparkTTS:
    ...
    @classmethod
    def from_pretrained(cls, model_id_or_path: str, device: torch.device = torch.device("cuda:0")):
        """
        Loads the SparkTTS model from a Hugging Face repo or a local directory.
        """
        if Path(model_id_or_path).is_dir():
            model_dir = Path(model_id_or_path)
        else:
            model_dir = Path(snapshot_download(repo_id=model_id_or_path))
